...
#%%%%%%%%%%%%  Neural Network Structure   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  def network_structure(self):
      ...
      self.strLoss = input('Give the loss function : ')
      self.strLoss = ''.join((self.strLoss).split()).upper()
      
      self.strLoss = self.strLoss + '_loss'
      self.strLoss_error = self.strLoss + '_error'
...
#%%%%  Partition the training data into mini-batches of a given size  %%%%%%%%%
  ...
  def batch_partitioning(self, training_data_one_hot, mini_batch_size):
    n = len(training_data_one_hot)
    np.random.shuffle(training_data_one_hot)
    mini_batches = [training_data_one_hot[k : k + mini_batch_size] for k in range(0, n, mini_batch_size)]
...
#%%%%%%%%%%%%%%%%%%%%%%%%%   Training the neural network     %%%%%%%%%%%%%%%%%%
  ...
  def train_network(self, training_data=None, training_data_one_hot=None, test_data=None, test_data_one_hot=None, 
                    strOptim='Momentum', gamma=0.0, beta_1 = 0.9, beta_2 = 0.999, rho=0.9, mini_batch_size=10, epochs=1, learning_rate=0.1, metrics=[]):
      ...
      for i in range(epochs):
          ...
          n, mini_batches = self.batch_partitioning(training_data_one_hot, mini_batch_size)
          for MB in mini_batches:
              self.update_mini_batch(i+1, MB, strOptim, learning_rate, n, gamma, beta_1, beta_2, rho)

    
          if 'train_acc' in metrics:
              T_accuracy = self.evaluate_accuracy(training_data)
              self.train_acc.append(T_accuracy)
              print('Epoch : {} _______________________ Train accuracy : {} '.format(i+1, T_accuracy), '\n')
          
          if 'test_acc' in metrics and test_data != None:
              V_accuracy = self.evaluate_accuracy(test_data)
              self.test_acc.append(V_accuracy)
              print('Epoch : {} _______________________ Test accuracy : {} '.format(i+1, V_accuracy), '\n')
          
          if 'train_loss' in metrics:
              T_loss = self.evaluate_loss(training_data_one_hot)
              self.train_loss.append(T_loss)
              print('Epoch : {} _______________________ Train loss : {} '.format(i+1, T_loss), '\n')
            
          if 'test_loss' in metrics and test_data_one_hot != None:
              V_loss = self.evaluate_loss(test_data_one_hot)
              self.test_loss.append(V_loss)
              print('Epoch : {} _______________________ Test loss : {} '.format(i+1, V_loss), '\n')
          
          print("===========================================================")  
        
      return self.train_acc, self.test_acc, self.train_loss, self.test_loss
...
#%%%%%%%%%%%%%%%%%%%%       Backpropagation Algorithm      %%%%%%%%%%%%%%%%%%%%
  ...
  def backpropagation_algorithm(self, x, y, strOptim, gamma):
    ...
    error = Loss.loss_error[self.strLoss_error](weighted_input_list[-1], activation_list[-1], y, Activ.fct_deriv[self.activ_list_derivative[-1]])
...
#%%%%%%%%%%%%%%%%%%%%%%%    Accuracy evaluation   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  def evaluate_accuracy(self, data):
      correct_numbers, wrong_numbers = 0, 0
      for i in range(len(data)):
          x, y = data[i]
          if np.argmax(self.run_network(x)) == y:
              correct_numbers += 1
          else:
              wrong_numbers += 1
      return 100 * correct_numbers / len(data) 
...
#%%%%%%%%%%%%%%%%%%%%%%%  Loss evaluation   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
  ...
  def evaluate_loss(self, data):
      ...
      for x, y in data:
          ...
          loss += Loss.loss[self.strLoss](a, y) / len(data)
      return loss    
